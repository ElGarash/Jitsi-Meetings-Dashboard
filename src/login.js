...
const createMeeting = async () => {
    ...
    document.getElementById("create-room").onsubmit = (event) =>
...
const sendRequest = async (url, method, body) => {
    try {
        ...
        console.error(e);
        console.log(
            "Some thing goes wrong with the request makes it fails.",
            e
        );
    }
};
...
const callJitsiAPI = async (room_name) => {
    try {
        ...
        window.participants = new Set(
            api.getParticipantsInfo().map((p) => p.participantId)
        );
